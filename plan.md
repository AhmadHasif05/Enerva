...
| ~~**P2**~~ ✅ | ~~**Audit & remove orphaned `model/` files** left from the screen trim — `Club.kt`, `Challenges.kt`, `Notifications.kt`, `Workout.kt` (and verify `Friends.kt`).~~ Done — all five (incl. `Friends.kt`) confirmed unreferenced and deleted; build still green. | Dead code inflates the surface area and confuses readers. |
| ~~**P2**~~ ✅ | ~~**Add a smoke test suite.**~~ Done — JVM unit tests for repository logic (conversation-id derivation, follow-doc-id sanitisation, reel mappers) + Room in-memory DAO instrumented tests (User/Message/Activity: queries, ordering, idempotent upsert, rename propagation, directory scoping). Templates removed. 13 unit tests (`testDebugUnitTest`) + 19 DAO tests (`connectedDebugAndroidTest`) all green. | "Has tests" is a baseline signal of engineering maturity. |
| ~~**P2**~~ ✅ | ~~**Standardise the brand.**~~ Done — the user-visible `app_name` was the matric string `a211198_Hasif_DrNelson_Project2`; set to **Enerva**. `RunTrackApplication`/`RunTrackTheme` kept as the documented internal codename. The Gradle module + package keep the graded-assignment name (also wired to `google-services.json`, so renaming the `applicationId` would break Firebase). | Consistent naming reads as intentional. |
